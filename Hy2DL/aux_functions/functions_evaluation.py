...
def rmse(df_results: Dict[str, pd.DataFrame], average: bool = True) -> np.array:
    ...
    df_results : Dict[str, pd.DataFrame]
        Dictionary, where each key is associated with a basin_id and each item is a pandas DataFrame.
        Each dataframe should contain at least two columns: 'y_sim' for the simulated values and 'y_obs'
        for the observed values.
    average : bool
        True if one wants to average the RMSE over all the basins (items of the dictionary), or False
        if one wants the value for each one.

    Returns
    -------
    error: np.array
        If average==True returns one value for all basins. If average==False returns the RMSE for each element.
    """
    errors = []
    # Iterate over each element (basin) of the dictionary
    for basin in df_results.values():
        # Read values
        y_sim = basin['y_sim'].values 
        y_obs = basin['y_obs'].values
        ...
        # Calculate RMSE
        if y_sim.size > 1 and y_obs.size > 1:
            errors.append(np.sqrt(np.mean((y_sim - y_obs)**2)))
        else:
            errors.append(np.nan)
                    
    return np.nanmedian(errors) if average else np.asarray(errors)